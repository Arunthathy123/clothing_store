...
const Testimonials = () => {
  ...
  useEffect(() => {
    const fetchTestimonials = async () => {
      try {
        const response = await fetch(JSON_FILES.TESTINOMIAL_DATA);
    ...
    <section className="py-10 mb-10">
      <div className="container mx-auto px-4">
        <div className="text-center mb-10 max-w-2xl mx-auto flex flex-col gap-3">
          <p data-aos="fade-up" className="text-sm text-gray-900">What our customers are saying</p>
          <h2 data-aos="fade-up" className="text-3xl font-bold text-gray-900">Testimonials</h2>
          <p data-aos="fade-up" className="text-xs text-gray-400">Hear what our happy clients have to say about our services.</p>
        </div>
            ...
            {testimonials.map(({ id, name, text, img }) => (
              <div key={id} className="px-4">
                <div className="bg-[#e9eeff] shadow-lg rounded-xl p-6 flex flex-col items-start relative">
                  <img src={img} alt={name} className="rounded-full w-20 h-20 mb-4" />
